    ...
    return (
        <header className={`${!isBuild ? 'absolute top-0 left-0 pt-4 md:pt-6 px-4 md:px-8' : 'relative p-4 md:p-6'} z-[60] w-full flex justify-center pointer-events-none transition-all duration-300`}>
            ...
            <div className="relative w-full max-w-7xl pointer-events-auto">
                ...
                <div className={`relative z-20 flex items-center justify-between px-5 md:px-7 py-3 md:py-4 rounded-xl border-[1.5px] border-dashed backdrop-blur-xl transition-all duration-500 ease-in-out ${isDarkMode
                    ...
                    <div className="hidden md:flex items-center gap-4">
                        ...
                        <button
                            onClick={openScan}
                            className={`flex items-center gap-2 px-5 py-2 ml-2 rounded-sm font-bold uppercase tracking-widest text-[11px] transition-all duration-300 active:scale-95 ${isDarkMode
                ...
                {isMobileMenuOpen && (
                    <div className={`absolute left-4 right-4 md:hidden z-10 pt-8 pb-5 px-6 rounded-b-xl border-[1.5px] border-t-0 border-dashed backdrop-blur-xl flex flex-col gap-1 -mt-4 animate-in slide-in-from-top-8 fade-in duration-400 ease-out shadow-2xl ${isDarkMode ? 'bg-slate-900/95 border-slate-600' : 'bg-[#faf9f6]/95 border-slate-300'
                        }`}>
                        ...
                        {displayMode === 'auto' && (
                            <button
                                ...
                                className="flex items-center gap-4 py-3 text-sm font-bold uppercase tracking-[0.15em] text-slate-500 hover:text-[var(--header-primary-color)] transition-colors w-full text-left"
                            >
                                {isDarkMode ? <Sun className="w-5 h-5 text-amber-500" /> : <Moon className="w-5 h-5" />}
                        ...
                        <Link
                            ...
                            className="flex items-center gap-4 py-3 text-sm font-bold uppercase tracking-[0.15em] text-slate-500 hover:text-[var(--header-primary-color)] transition-colors w-full"
                        >
                            <History className="w-5 h-5" />
                            Riwayat
                        </Link>
                        ...
                        <button
                            ...
                            className={`mt-3 flex items-center justify-between w-full p-4 rounded-sm font-bold uppercase tracking-widest text-xs transition-transform active:scale-95 ${isDarkMode ? 'bg-[var(--header-primary-color)] text-slate-900' : 'bg-[var(--header-primary-color)] text-white shadow-md'
                                }`}
                        >
            ...
            {/* Backdrop transparan untuk deteksi klik di luar menu mobile */}
            {isMobileMenuOpen && (
                <div
                    className="fixed inset-0 z-0 md:hidden pointer-events-auto bg-black/5 backdrop-blur-[1px] transition-opacity"
                    onClick={() => setIsMobileMenuOpen(false)}
                    aria-hidden="true"
                />
            )}
