...
const Ten = ({ isDarkMode, isBuild, totalCart, summary, selectedOutlet }: Props) => {
    ...
    // Sembunyikan panel secara penuh jika keranjang belum terisi
            ...
            `}>
                ...
                <div className="max-w-screen-xl mx-auto px-5 py-4 flex justify-between items-center">
                    ...
                    <div className="flex flex-col justify-center">
                        <p className={`text-[11px] font-extrabold uppercase tracking-widest mb-1 ${isDarkMode ? "text-slate-400" : "text-slate-500"
                            }`}>
                            Keranjang ({totalCart})
                        </p>
                        <p className={`text-xl font-black tracking-tighter leading-none ${isDarkMode ? "text-white" : "text-slate-900"
                            }`}>
                            {formatIDR(summary)}
                        </p>
                    </div>

                    {/* Bagian Kanan: Sporty Italic Button dengan Proteksi YIQ yang Cerdas */}
                    <HandleCheckout
