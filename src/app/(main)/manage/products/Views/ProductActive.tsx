...
import { Edit, Trash2Icon, QrCode } from "lucide-react"; // Tambah QrCode icon
...
import ProductFormModalContent from "../Components/ProductFormModalContent";
...
export default function ProductActive() {
    ...
    const fetchProducts = useCallback(async () => {
        ...
        setError('');
        try {
            const res = await Get<{ success: boolean; data: ProductResponse; meta: Meta }>(`client/products${queryString}`);
            if (res?.success) {
    ...
    const handleFormSubmit = async (formData: FormData, id: number | null) => {
        setLoading(true);
        try {
            const endpoint = id ? `client/products/${id}` : 'client/products';
    ...
    const onDelete = async (id: number | null) => {
        setLoading(true);
        try {
            const res = await Delete(`client/products/${id}`);
            if (res) {
                fetchProducts();
                handleCloseModal();
    ...
    const handleToggleActive = async (row: ProductsType) => {
        setLoading(true);
        try {
            // Kita bikin endpoint khusus untuk toggle status biar gampang dan cepat di backend
            const formData = new FormData();
            formData.append('is_active', row.is_active ? '0' : '1');

            const res = await Post(`client/products/${row.id}/toggle-active`, formData);
            if (res) {
                fetchProducts();
                setShowAlert({ type: 'success', message: `Status produk ${row.name} berhasil diubah!`, isOpen: true });
            }
        } catch (err: any) {
    ...
    const columns: Column<ProductsType>[] = useMemo(() => [
        ...
        // MENGGABUNGKAN IS_ACTIVE DAN IS_AVAILABLE MENJADI SATU TOGGLE SWITCH
        {
            key: "is_active",
            label: "Status Aktif",
            align: "center",
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="mt-6">
                ...
                {/* 1. TAMPILAN MOBILE (CARDS) - Muncul di HP, Sembunyi di Laptop */}
                <div className="block md:hidden space-y-4">
                    {loading ? (
                        <div className="text-center py-6 text-slate-500 font-semibold animate-pulse">Memuat data...</div>
                    ) : products.length === 0 ? (
                        <div className="text-center py-6 text-slate-500 font-semibold">Data produk tidak ditemukan</div>
                    ) : (
                        products.map((row) => (
                                    ...
                                    <div className="bg-slate-50/70 p-2.5 rounded-lg text-xs space-y-1.5 border border-slate-100 mt-1">
                                        {row.variants.map((v, i) => (
                                            <div key={i} className="flex justify-between items-center border-b border-slate-200/50 pb-1 last:border-0 last:pb-0">
                                                <span className="font-semibold text-slate-700">{v.name}</span>
                                                <span className="text-emerald-600 font-bold bg-emerald-50 px-2 py-0.5 rounded-full">Stok: {v?.product_variant_stock ?? 0}</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
                ...
                {/* 2. TAMPILAN DESKTOP (TABLE) - Sembunyi di HP, Muncul di Laptop */}
