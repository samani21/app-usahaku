...
const ModalAddOrder = ({ onClose, addToast, outlets, handleSubmit }: Props) => {
    ...
    const getProduct = async () => {
        setLoading(true);
        try {
            const res = await Get<{ success: boolean, data: { products: ProductsType[], promos: PromoType[] } }>(`employee/orders/list-products?outlet=${newOrder.outletId}`);
            if (res?.success) {
    ...
    const handleSubmitOrder = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await Post<any, FormData>('employee/orders', formData);
            if (res?.success) {
